...
export class ContestUtil {
  ...
  bestSquareLogo(logos: FileReferenceJSON[]|undefined, size: number): FileReferenceJSON | undefined {
    if (!logos || size < 1)
      return undefined;
  ...
  bestLogo(logos: FileReferenceJSON[]|undefined, width: number, height: number): FileReferenceJSON | undefined {
    ...
    for (var i = 0; i < submissions.length; i++) {
      ...
      if (time && time >= 0 && submissions[i].problem_id == problem_id) {
        ...
        if (judgements != null && judgements.length > 0) {
          let jt = this.findById(contest.getJudgementTypes(), judgements[judgements.length - 1].judgement_type_id);
          if (jt != null) {
            if (jt.solved) {
